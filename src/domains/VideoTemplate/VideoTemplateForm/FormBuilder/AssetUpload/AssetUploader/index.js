...
export default function AssetUploader({
  ...
  const [src, setSrc] = useState(
    typeof asset?.src === "object" ? false : Boolean(asset?.src)
  );
  ...
  const handleAssetUpload = async (e) => {
    if (type !== "folder") {
      ...
      setAssets(
        temp
          .filter(({ name }) => assetsName.includes(name))
  ...
  const handleUpload = async (file) => {
    try {
      ...
      setLoading(true);
      const task = upload(
        `staticAssets/${Date.now()}${file.name.substr(
      ...
      <Box m={1}>
        <Box my={1}>
          ...
          <Typography
            ...
            {loading
              ? ` Uploading: ${progress} `
              : `${type === "folder" ? "Asset Folder" : asset?.name}`}
          </Typography>
          {loading && (
            <Button
