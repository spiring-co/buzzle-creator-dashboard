...
export default () => {
  ...
  const getCountDay = async (update, start) => {
    await fetch(
      `http://localhost:5000/stats/count?dateUpdated=${update}&dateStarted=${start}`
    )
  ...
  const getDataFromQuery = async (query) => {
    console.log("yes");
    await fetch(
      `http://localhost:5000/stats/countTitles?page=${1}&size=${100}&sortBy=${`dateUpdated=>=${startDate}&dateUpdated=<=${
        endDate || startDate
      }`}&orderBy=${"dateUpdated"}`
    )
      .then((response) => response.json())
