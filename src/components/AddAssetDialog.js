...
export default (props) => {
      ...
      case "image":
        props.editAsset
          ? props.editAssetValue({
              ...
              layerName,
              extension: "png",
              src: "https://dummyimage.com/500x500/329da8/fff",
            })
          : props.addAsset({
              type,
              layerName,
  ...
  useEffect(() => {
    ...
    if (values?.layerName !== "") {
      var { type, width, height, maxLength, label } = editableLayers[
        layerNames.indexOf(values.layerName)
      ];
      console.log(type);
      setType(type);
      setHeight(height);
    ...
    <Dialog
      ...
      <form onSubmit={handleSubmit}>
        <DialogTitle id="form-dialog-title">
          {props.editAsset ? "Edit Asset" : "Add Asset"}
        </DialogTitle>
        <DialogContent>
          <FormControl
            fullWidth
            ...
            <Select
              ...
              {layerNames.map((item, index) => {
                if (
                  props.usedLayers.includes(item) &&
